...
function makeKerberosClient(authContext, callback) {
    ...
    performGssapiCanonicalizeHostName(hostAddress.host, mechanismProperties, (err, host) => {
        ...
        const initOptions = {};
        if (password != null) {
            if (password || !process.argv.includes('--debugUseKrbDropEmptyPassword')) {
        ...
        if (initOptions.user && process.argv.includes('--debugUseKrbDomain')) {
            const [user, domain] = initOptions.user.split('@');
            initOptions.user = user;
            initOptions.domain = domain;
        }
        console.error(`initializeClient(${JSON.stringify(spn)}, ${JSON.stringify({username: initOptions.user, domain: initOptions.domain, passwordLength: initOptions.password&&initOptions.password.length})})`);

...
export interface AsyncRewriter {
  process: (code: string) => string;
  runtimeSupportCode: () => string;
}

export interface RunTestsOptions {
  newAsyncRewriter(): Promise<AsyncRewriter> | AsyncRewriter;
  hooks: {
    describe: (heading: string, fn: () => void) => void;
    context: (heading: string, fn: () => void) => void;
    it: (heading: string, fn: () => void | Promise<void>) => void;
...
function runAsyncRewriterTests({
  ...
  beforeEach(async function () {
    ...
    asyncWriter = await newAsyncRewriter();
  ...
  context('domain support', function () {
    ...
    beforeEach(async function () {
      tmpdir = path.join(os.tmpdir(), `async-transpiler-test-${Date.now()}`);
    ...
    it('works fine when run inside a Node.js domain context', async function () {
      const runtimeSupportCodeFilename = path.join(tmpdir, 'runtimeSupport.js');
      await fs.writeFile(
        runtimeSupportCodeFilename,
        asyncWriter.runtimeSupportCode()
      );
      // Write the test fixture inline so that this works in both CommonJS and
      // ES module contexts (where `__dirname` may be unavailable).
      const fixtureFilename = path.join(tmpdir, 'with-domain.js');
      await fs.writeFile(
        fixtureFilename,
        `'use strict';
...
`
      ...
      await execFile(process.execPath, [fixtureFilename], {
        timeout: 15_000,
        env: {
          ...process.env,
          ASYNC_REWRITER_RUNTIME_SUPPORT_CODE: runtimeSupportCodeFilename,
        },
      });
    });
  });

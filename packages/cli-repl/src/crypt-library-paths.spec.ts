...
describe('getCryptLibraryPaths', () => {
  ...
  const expectedVersion = { version: BigInt('0x0001000000000000'), versionStr: 'mongo_crypt_v1-dummy' };

  beforeEach(async function() {
    events = [];
    bus = new EventEmitter();
    bus.on('mongosh:crypt-library-load-found', (ev) => events.push(['mongosh:crypt-library-load-found', ev]));
    bus.on('mongosh:crypt-library-load-skip', (ev) => events.push(['mongosh:crypt-library-load-skip', ev]));

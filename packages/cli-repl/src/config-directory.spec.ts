...
describe('ConfigAndLogDirectoryManager', () => {
  let onError: Function;
  let onUpdateConfig: Function;
  let onNewConfig: Function;
  let base: string;
  let manager: ConfigAndLogDirectoryManager<ExampleConfig>;

  beforeEach(() => {
    base = path.resolve(__dirname, '..', '..', '..', 'tmp', 'test', `${Date.now()}`, `${Math.random()}`);
    manager = new ConfigAndLogDirectoryManager(base);

...
describe('SnippetManager', () => {
  ...
  beforeEach(async() => {
    ...
    httpServer = http.createServer((req, res) => {
      ...
      switch (req.url) {
        case '/index2.bson.br':
        case '/index.bson.br': {
          res.writeHead(200, { 'Content-type': 'application/x-mongosh-snippet-index' });
          const compress = zlib.createBrotliCompress();
          compress.end(bson.serialize(req.url === '/index.bson.br' ? indexData : indexData2));
          compress.pipe(res);
          break;
        }
          ...
          res.end(JSON.stringify(
            {
              ...
              versions: {
                '5.7.1': {
                  ...
                  _id: 'buffer@5.7.1',
                  dist: {
                    integrity: 'sha512-YG4pBaPbbnuPvoy8kXTU6fP0rwlSfCOuAzCIe9CGNLIWCvRSiJiQdX1fqws4CaRWI79v/ZjOur6yiUwX4Cwy9Q==',
        ...
        case '/npm-7.15.0.tgz': {
          ...
          const source = createReadStream(path.join(__dirname, '..', 'test', 'fixtures', '.' + req.url));
          source.pipe(res);
          break;
        }
        case '/notindexfile': {
  ...
  it('manages packages on disk', async() => {
    ...
    expect(contextObject.load.getCalls().map(({ args }: any) => args)).to.deep.equal([
      [path.resolve(tmpdir, 'snippets', 'node_modules', 'bson', 'lib', 'bson.js')]
    ]);
...

    {
      ...
      await eventually(async() => {
        ...
        const hiddenPkgLock = path.resolve(tmpdir, 'snippets', 'node_modules', '.package-lock.json');
        let exists = false;
        try {
    ...
    expect(busMessages).to.deep.equal([
      { ev: 'mongosh-snippets:snippet-command', data: { args: [ 'install', 'bson-example' ] } },
      { ev: 'mongosh-snippets:spawn-child', data: { args: [ 'npm', '--version' ] } },
      { ev: 'mongosh-snippets:fetch-index-done' },
      { ev: 'mongosh-snippets:npm-lookup', data: { existingVersion: '<version>' } },
      { ev: 'mongosh-snippets:package-json-edit-error', data: { error: 'ENOENT' } },
      { ev: 'mongosh-snippets:spawn-child', data: { args: [ ...npmlog, 'install', '--save', 'bson@*' ] } },
      { ev: 'mongosh-snippets:load-snippet', data: { source: 'install', name: 'bson' } },
      { ev: 'mongosh-snippets:snippet-command', data: { args: [ 'load-all' ] } },
      { ev: 'mongosh-snippets:load-snippet', data: { source: 'load-all', name: 'bson' } },
      { ev: 'mongosh-snippets:snippet-command', data: { args: [ 'load-all' ] } },
      { ev: 'mongosh-snippets:load-snippet', data: { source: 'load-all', name: 'bson' } },
      { ev: 'mongosh-snippets:snippet-command', data: { args: [ 'ls' ] } },
      { ev: 'mongosh-snippets:spawn-child', data: { args: [ ...npmlog, 'ls', '--depth=0' ] } },
      { ev: 'mongosh-snippets:snippet-command', data: { args: [ 'outdated' ] } },
      { ev: 'mongosh-snippets:spawn-child', data: { args: [ ...npmlog, 'outdated' ] } },
      { ev: 'mongosh-snippets:snippet-command', data: { args: [ 'update' ] } },
      { ev: 'mongosh-snippets:spawn-child', data: { args: [ ...npmlog, 'update', '--save' ] } },
      { ev: 'mongosh-snippets:snippet-command', data: { args: [ 'outdated' ] } },
      { ev: 'mongosh-snippets:spawn-child', data: { args: [ ...npmlog, 'outdated' ] } },
      { ev: 'mongosh-snippets:snippet-command', data: { args: [ 'uninstall', 'bson-example' ] } },
      { ev: 'mongosh-snippets:spawn-child', data: { args: [ ...npmlog, 'uninstall', '--save', 'bson' ] } },
      { ev: 'mongosh-snippets:snippet-command', data: { args: [ 'ls' ] } },
      { ev: 'mongosh-snippets:spawn-child', data: { args: [ ...npmlog, 'ls', '--depth=0' ] } }
    ]);
  });
  ...
  context('without a recent npm in $PATH', () => {
    ...
    it('downloads npm if asked to', async() => {
      ...
      const fullPkgJson = JSON.parse(await fs.readFile(path.join(installdir, 'package.json'), 'utf8'));
      expect(Object.keys(fullPkgJson.dependencies)).to.deep.equal(['bson', 'npm']);
      ...
      snippetManager = makeSnippetManager();
      expect((await snippetManager.runSnippetCommand(['ls']))).to.include('mongosh:bson-example@4.4.0');
      expect(evaluationListener.onPrompt).to.have.callCount(2);
    });
  ...
  describe('interruption support', () => {
    ...
    it('kills npm commands when interrupted', async() => {
      ...
      let pid = -1;
      await eventually(async() => {
        pid = +await fs.readFile(path.join(installdir, 'infinite-sleep.pid'), 'utf8');
      });

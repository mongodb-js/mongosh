...
describe('uri-generator.generate-uri', function() {
  ...
  context('when a full URI is provided', function() {
    ...
    context('when additional options are provided', function() {
      ...
      context('when providing gssapiServiceName', function() {
        context('and the URI does not include SERVICE_NAME in authMechanismProperties', function() {
        ...
        context('and the URI includes SERVICE_NAME in authMechanismProperties', function() {
    ...
    context('when providing a URI with the legacy gssapiServiceName query parameter', function() {
  ...
  context('when a URI is provided without a scheme', function() {
    ...
    context('when no additional options are provided with empty db', function() {
      const uri = '192.0.0.1:27018/';
      const options = { connectionSpecifier: uri };
    ...
    context('when no additional options are provided with db with special characters', function() {
    ...
    context('when additional options are provided', function() {
      ...
      context('when providing explicit serverSelectionTimeoutMS (different case)', function() {
        const uri = 'mongodb://127.0.0.2/foo?SERVERSELECTIONTIMEOUTMS=10';
        const options = { connectionSpecifier: uri };

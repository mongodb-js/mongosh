...
if [[ "$DISABLE_OPENSSL_SHARED_CONFIG" == "true" ]] && [[ ! "$E2E_TASK_NAME" =~ openssl(3|11) ]]; then
  ...
  # `rh-allow-sha1-signatures` is present which is not recognizable to the OpenSSL version bundled with Node.js
  # and hence the mongosh binary fails to run. Explicitly on those hosts we
  # disable effect of --openssl-shared-config flag which is pushed by boxednode
  # when bundling Node.js
  export OPENSSL_CONF=""
fi
